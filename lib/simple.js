...
END:VCARD`.trim()
        ...
        return await conn.sendMessage(jid, {
            contacts: {
                displayName: (contacts.length > 1 ? `1.000.000.000.000 kontak` : contacts[0].displayName) || null,
                contacts,
            }
        },
        {
...
/*exports.smsg = (conn, m, hasParent) => {
    ...
    m.react = (emote) => conn.sendMessage(m.chat, { react: { text: emote, key: m.key }})
     ...
     * @param {Object} options
     */
     m.mediaReply = async (text, chatId, options) => {
