...
const authFolder = `${opts._[0] || global.session}`
...
cron.schedule('00 00 * * *', () => {
   global.db.data.users.filter(v => v.limit < global.limit && !v.premium).map(v => v.limit = global.limit)
}, {
   scheduled: true,
