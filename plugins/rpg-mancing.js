let handler = async (m, { conn,usedPrefix, command, text, args }) => {
       ...
       const editedMessage = async (key, convert, timeout) => {
        ...
        switch (type) {
	          case 'easy':
                                  ...
                                  if (stamina < 10) return m.reply(`Stamina kamu rendah!\nSilahkan isi dulu dengan mengetik *#makan*`)
...
`.trim()
                                 ...
                                 global.db.data.users[m.sender].stamina -= rbrb12
                                
                                setTimeout(() => {
                                m.reply(`Yuk mancing mania level easy lagi`)
                                }, 28800000)
                                const _easy = await conn.reply(m.chat, '_Sedang Memancing..._', m)
                                const easy = [
                                    { text: `Kamu nunggu sambil ${pickRandom(global.menunggu)}`, timeout: 5000 },
                                    { text: 'Strike..!!', timeout: 10000 },
                                    { text: `${hsl}`, timeout: 20000 }
                                ];
                                for (let i = 0; i < easy.length; i++) {
                                    editedMessage(_easy, easy[i].text, easy[i].timeout)
                                }
                 ...
                 break
	          case 'normal':
                                  let __timerl = (new Date - global.db.data.users[m.sender].lastmancingnormal)
                                  let _timerl = (28800000 - __timerl)
                                  let timerl = clockString(_timerl) 
                                  if (pancingan == 0 || umpan == 0) return m.reply('*Kamu belum memiliki Pancingan dan Umpan, Silahkan beli dulu..*')
                                  if (stamina < 15) return m.reply(`Stamina kamu rendah!\nSilahkan isi dulu dengan mengetik *#makan*`)
                                  if (new Date - global.db.data.users[m.sender].lastmancingnormal > 28800000) {
                                  ...
                                  let randomakud14 = `${Math.floor(Math.random() * 15)}`
...
`.trim()
                                 ...
                                 global.db.data.users[m.sender].stamina -= rbrbs14 
                                 
                                setTimeout(() => {
                                m.reply(`Yuk mancing mania level normal lagi`)
                                }, 28800000)
                                const _normal = await conn.reply(m.chat, '_Sedang Memancing..._', m)
                                const normal = [
                                    { text: `Kamu nunggu sambil ${pickRandom(global.menunggu)}`, timeout: 5000 },
                                    { text: 'Strike..!!', timeout: 10000 },
                                    { text: `${hsls}`, timeout: 20000 }
                                ];
                                for (let i = 0; i < normal.length; i++) {
                                    editedMessage(_normal, normal[i].text, normal[i].timeout)
                                }
                 ...
                 break 
	          case 'hard':
                                  let __timerh = (new Date - global.db.data.users[m.sender].lastmancinghard)
                                  let _timerh = (28800000 - __timerh)
                                  let timerh = clockString(_timerh) 
                                  if (pancingan == 0 || umpan == 0) return m.reply('*Kamu belum memiliki Pancingan dan Umpan, Silahkan beli dulu..*')
                                  if (stamina < 20) return m.reply(`Stamina kamu rendah!\nSilahkan isi dulu dengan mengetik *#makan*`)
                                  if (new Date - global.db.data.users[m.sender].lastmancinghard > 28800000) {
                                  ...
                                  let randomakur14 = `${Math.floor(Math.random() * 20)}`
...
`.trim()
                                 ...
                                 global.db.data.users[m.sender].stamina -= rbrbsh14

                                setTimeout(() => {
                                m.reply(`Yuk mancing mania level hard lagi`)
                                }, 28800000)
                                const _hard = await conn.reply(m.chat, '_Sedang Memancing..._', m)
                                const hard = [
                                    { text: `Kamu nunggu sambil ${pickRandom(global.menunggu)}`, timeout: 5000 },
                                    { text: 'Strike..!!', timeout: 10000 },
                                    { text: `${hslsh}`, timeout: 20000 }
                                ];
                                for (let i = 0; i < hard.length; i++) {
                                    editedMessage(_hard, hard[i].text, hard[i].timeout)
                                }
                 ...
                 break
	          case 'extreme':
                                  let __timere = (new Date - global.db.data.users[m.sender].lastmancingextreme)
                                  let _timere = (28800000 - __timere)
                                  let timere = clockString(_timere) 
                                  if (pancingan == 0 || umpan == 0) return m.reply('*Kamu belum memiliki Pancingan dan Umpan, Silahkan beli dulu..*')
                                  if (stamina < 25) return m.reply(`Stamina kamu rendah!\nSilahkan isi dulu dengan mengetik *#makan*`)
                                  if (new Date - global.db.data.users[m.sender].lastmancingextreme > 28800000) {
                                  ...
                                  let randomakue14 = `${Math.floor(Math.random() * 25)}`
...
`.trim()
                                 ...
                                 global.db.data.users[m.sender].stamina -= rbrbshe14

                                setTimeout(() => {
                                m.reply(`Yuk mancing mania level extreme lagi`)
                                }, 28800000)
                                const _extreme = await conn.reply(m.chat, '_Sedang Memancing..._', m)
                                const extreme = [
                                    { text: `Kamu nunggu sambil ${pickRandom(global.menunggu)}`, timeout: 5000 },
                                    { text: 'Strike..!!', timeout: 10000 },
                                    { text: `${hslse}`, timeout: 20000 }
                                ];
                                for (let i = 0; i < extreme.length; i++) {
                                    editedMessage(_extreme, extreme[i].text, extreme[i].timeout)
                                }
