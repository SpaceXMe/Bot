...
conn.level = global.db.data.users[m.sender]
  ...
  if (global.db.data.users[m.sender].centaur == 0) return m.reply(`Kamu belum memiliki Centaur Silahkan beli dulu..\n\nKetik: *${usedPrefix}petshop centaur* untuk membeli centaur`)
  ...
  m.reply(`*Pet Kamu* (🐴centaur level ${global.db.data.users[m.sender].centaur}) ⚔️menantang 🐴centaurnya *${conn.getName(lawan)}* (🐴centaur level ${global.db.data.users[lawan].centaur}) lagi berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang🎮.`)
