...
await conn.sendPresenceUpdate('composing', m.chat)
    ...
    if (!text) {
        var pesan = "Harap aktif di grup karena akan ada pembersihan member setiap hari"
    } else {
        var pesan = text
    }
    var sum
    ...
    if (total == 0) return conn.reply(m.chat, `*Tidak ditemukan sider di group ini.*`, m)
    conn.reply(m.chat, `Terdapat *${total}/${sum}* sider di grup ${await conn.getName(m.chat)}\n\nAlasan :\n1. Tidak aktif selama lebih dari 7 hari\n2. Baru join tetapi tidak pernah nimbrung\n\n_“${pesan}”_\n\n*LIST SIDER :*\n${sider.map(v => '  ○ @' + v.replace(/@.+/, '' + typeof global.db.data.users[v] == "undefined" ? ' Sider ' : ' Off ' + msToDate(milliseconds * 1 - global.db.data.users[v].lastseen))).join('\n')}`, m, {
        contextInfo: {
            mentionedJid: sider
        }
    })
...
handler.tags = ['admin']
handler.command = /^(gcsider|getsider)$/i
handler.group = true
handler.admin = true
