const { currency, pickRandom, clockString } = require('../lib/myfunc')
...
let handler = async (m, { conn, args, usedPrefix, isOwner }) => {
    ...
    try {
        ...
        let count = args[0].replace(/\./g, '')
        count = count ? /all/i.test(count) ? Math.floor(global.db.data.users[m.sender].money / buatall) : parseInt(count) : args[0] ? parseInt(count) : 1
        count = Math.max(1, count)
        if (args.length < 1) return conn.reply(m.chat, usedPrefix + 'judi <jumlah>\n ' + usedPrefix + 'judi 1000', m)
        if (global.db.data.users[m.sender].money >= count * 1) {
            global.db.data.users[m.sender].money -= count * 1
            if (Aku > Kamu) {
                conn.reply(m.chat, `*Bot Roll:* ${Aku}\n*Kamu Roll:* ${Kamu}\n\nKamu *KALAH*, kamu kehilangan ${currency(count)} money`.trim(), m)
            } else if (Aku < Kamu) {
                global.db.data.users[m.sender].money += count * 2
                conn.reply(m.chat, `*Bot Roll:* ${Aku}\n*Kamu Roll:* ${Kamu}\n\nKamu *MENANG*, kamu Mendapatkan ${currency(count * 2)} money`.trim(), m)
            } else {
                global.db.data.users[m.sender].money += count * 1
                conn.reply(m.chat, `*Bot Roll:* ${Aku}\n*Kamu Roll:* ${Kamu}\n\nKita *SERI*, kamu Mendapatkan ${currency(count * 1)} money`.trim(), m)
            }
        } else conn.reply(m.chat, `Money kamu tidak cukup untuk melakukan judi sebesar ${currency(count)} money`.trim(), m)
