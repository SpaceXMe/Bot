...
let handler = async (m, { conn }) => {
  m.reply('Updating anime...')
	let listAnime = []
	let lastAnime = listAnime
	setInterval(async () => {
...
handler.help = ['updateanime']
handler.tags = ['anime']
handler.command = /^(update(anime|nime)|(anime|nime)update)$/i
handler.limit = 10
...
async function getLatestAnime() {
	let html = (await axios.get('https://anoboy.monster/')).data
