...
let { pickRandom, msToDate } = require('../lib/myfunc')
...
async function handler(m, { conn, args, text , usedPrefix, command }) {
  ...
  try {
    ...
    if(new Date - user.lastmisi <= cooldown) return m.reply(`Kamu sudah menjalankan misi, tunggu selama *${msToDate(timers)}*`)
...
handler.before = async m => {
  ...
  let timers = (cooldown - (new Date - user.lastmisi))
  if(new Date - user.lastmisi <= cooldown) return m.reply(`Kamu sudah melakukan misi, mohon tunggu *${msToDate(timers)}*`)
  if(!user.skill) return m.reply(`Ketik *${usedPrefix}selectskill* untuk memilih skill`)
