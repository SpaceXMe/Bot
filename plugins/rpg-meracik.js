...
`.trim()
                     ...
                     case 'pedang':
                                     ...
                                     if (iron == 0 || kayu == 0 || string == 0) return m.reply('*Pastikan kamu memiliki semua bahan yang dibutuhkan*\n\n*10 String*\n*500 Kayu*\n*5 Iron*')
...
`.trim()
                     ...
                     break
                     case 'armor':
                                     let irona = global.db.data.users[m.sender].iron
                                     let kayua = global.db.data.users[m.sender].kayu
                                     let batua = global.db.data.users[m.sender].batu

                                     if (irona == 0 || kayua == 0 || batua == 0) return m.reply(`Pastikan kamu memiliki semua bahan yang dibutuhkan!\n\n*Batu ${batua} / 150*\n*Kayu ${kayua} / 500*\n*Iron ${irona} / 200*`)
                                     if (user.armor < 1) {
                                     if (global.db.data.users[m.sender].iron > 199) {
                                 	if (global.db.data.users[m.sender].kayu > 499) {
                                 	if (global.db.data.users[m.sender].batu > 149) {
                                     let _ironna = `${Math.floor(Math.random() * 200)}`.trim()
                                     let _kayuna = `${Math.floor(Math.random() * 500)}`.trim()
                                     let _batuna = `${Math.floor(Math.random() * 150)}`.trim()
                                     let ironna = (_ironna * 1)
                                     let kayuna = (_kayuna * 1)
                                     let batuna = (_batuna * 1)
                                     global.db.data.users[m.sender].iron -= ironna * 1
                                     global.db.data.users[m.sender].kayu -= kayuna * 1
                                     global.db.data.users[m.sender].batu -= batuna * 1
                                     global.db.data.users[m.sender].armor += 1
                                     let srcnga = `
Berhasil meracik *Armor*:
-${ironna} Iron
-${kayuna} Kayu
-${batuna} Batu

Selamat kamu mendapatkan *Armor*: 
+1
`.trim()
                                     setTimeout(() => {
                                          conn.reply(m.chat, srcnga, m)
                                      }, timeoutar)
                                     setTimeout(() => {
                                          conn.reply(m.chat, 'Mohon tunggu sedang crafting armor', m)
                                      }, timeoutoh)
                                 } else m.reply(`Pastikan batu kamu *150* untuk bisa crafting armor`)
                             } else m.reply(`Pastikan kayu kamu *500* untuk bisa crafting armor`)
                          } else m.reply(`Pastikan iron kamu *200* untuk bisa crafting armor`)
                       } else m.reply(`Kamu sudah memiliki *Armor*, tidak bisa crafting kembali..\nUpgrade *Armor* kamu dengan mengetik:\n\n*${usedPrefix}upgrade armor*`)
                     break
                     case 'pickaxe':
                                     let ironp = global.db.data.users[m.sender].iron
                                     let kayup = global.db.data.users[m.sender].kayu
                                     let batup = global.db.data.users[m.sender].batu
                                     let __waktutionp = (new Date - global.db.data.users[m.sender].lastpickaxeclaim)
                                     let _waktutionp = (600000 - __waktutionp)
                                     let waktutionp = clockString(_waktutionp)
                                     if (ironp == 0 || kayup == 0 || batup == 0) return m.reply(`Pastikan kamu memiliki semua\n*Iron ${user.iron} / 200*\n*Kayu ${user.kayu} / 200*\n*Batu ${user.batu} / 200*`)
                                     if (global.db.data.users[m.sender].pickaxe < 1) {
                                     if (global.db.data.users[m.sender].iron > 199) {
                                 	if (global.db.data.users[m.sender].kayu > 199) {
                                 	if (global.db.data.users[m.sender].batu > 199) {
                                     let _ironnp = `${Math.floor(Math.random() * 200)}`.trim()
                                     let _kayunnp = `${Math.floor(Math.random() * 200)}`.trim()
                                     let _batunp = `${Math.floor(Math.random() * 200)}`.trim()
                                     let ironnp = (_ironnp * 1)
                                     let kayunp = (_kayunnp * 1)
                                     let batunp = (_batunp * 1)
                                     global.db.data.users[m.sender].iron -= ironnp * 1
                                     global.db.data.users[m.sender].kayu -= kayunp * 1
                                     global.db.data.users[m.sender].batu -= batunp * 1
                                     global.db.data.users[m.sender].pickaxe += 1
                                     global.db.data.users[m.sender].pickaxedurability += 30
                                     let srcngs = `
Berhasil Crafting Pickaxe:
-${ironnp} Iron
-${kayunp} Kayu
-${batunp} Batu

Selamat kamu mendapatkan Pickaxe: 
+1 Wood Pickaxe
`.trim()
                                     setTimeout(() => {
                                          conn.reply(m.chat, srcngs, m)
                                     ...
                                     setTimeout(() => {
                                          conn.reply(m.chat, 'Mohon tunggu sedang crafting Pickaxe', m)
                                      }, timeoutig)
                                 } else m.reply(`Pastikan batu kamu *200* untuk bisa craft Pickaxe`)
                             } else m.reply(`Pastikan kayu kamu *200* untuk bisa craft Pickaxe`)
                          } else m.reply(`Pastikan iron kamu *200* untuk bisa craft Pickaxe`)
                       } else m.reply(`Kamu sudah crafting Pickaxe, tidak bisa craft kembali. Tetapi kamu bisa upgrade Pickaxe kamu dengan mengetik\n\n*${usedPrefix}upgrade pickaxe*`)
                     break
                     case 'weapon':
...
`.trim()
                     ...
                     break
                           default:
                                  return conn.reply(m.chat, `${usedPrefix + command} [ramuan | potion | string | iron | sword | pickaxe | weapon | armor | pancingan]\nContoh penggunaan: *${usedPrefix + command} ramuan*`, m)
                           }
                        }

handler.help = ['meracik <type>']
handler.tags = ['rpg']
handler.command = /^(craft|meracik|racik|membuat|buat)$/i
handler.limit = true
handler.group = true
