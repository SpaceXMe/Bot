...
*${usedPrefix + command} start* = start game`)
  ...
  if (args[0] == "money"){
    ...
    if(m.sender == conn.war[m.chat][0].user){
      if (args[1] != "undefined" && !isNaN(args[1])){
        ...
        if (args[1] < 100) return m.reply('*Minimal $100*')
        conn.war2[m.chat].money = args[1]
        return m.reply("*Berhasil menetapkan modal perang sebesar $" + Number(args[1]) + "*")
      }else {
  ...
  if (args[0] == "join"){
    ...
    if (global.db.data.users[m.sender].money < 50) return m.reply("*Uang kamu minimal $50 untuk bermain game ini.*")
    // FIRST PLAYER
    if (!(m.chat in conn.war)) {
      ...
      if (args[1]){
        if (args[1].toLowerCase() == "a"){
          if (conn.war2[m.chat].money == 0) return conn.reply(m.chat,`*Tolong @${conn.war[m.chat][0].user.split('@')[0]} tetapkan modal awal perang (minimal $100)*\n\n.war money 100`,m, {contextInfo : {mentionedJid : [conn.war[m.chat][0].user]}})
          return m.reply('a')
          if (global.db.data.users[m.sender].money < conn.war2[m.chat].money) return m.reply(`*Uang kamu minimal $${conn.war2[m.chat].money} untuk bermain game ini.*`)
          for (let i = 1 ; i < 5 ; i++) {
            if (conn.war[m.chat][i].user == ""){
              let exp = global.db.data.users[m.sender].exp
