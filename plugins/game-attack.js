      ...
      if (conn.war[m.chat][turn].hp <= 0) {
        ...
        if (turn < 5){
          ...
          if (playerTotal > 0 && playerTotal == playerKalah){
            ...
            conn.reply(m.chat, `*TEAM B MENANG KARENA TEAM A GOBLOK SEMUA*\n\n*TEAM A :*\n` + teamA.map((v, i )=> `${conn.war[m.chat][i].hp > 0 ? '❤️ ' : '☠️ ' }@${v.split('@')[0]} (- ${currency(Number(conn.war2[m.chat].money))})`).join`\n` + "\n\n*TEAM B :*\n" + teamB.map((v, i) => `${conn.war[m.chat][i+5].hp > 0 ? '❤️ ' : '☠️ ' }@${v.split('@')[0]} (+ ${currency(Number(conn.war2[m.chat].money))})`).join`\n`,m, {contextInfo: {
              mentionedJid: teamAB
            }})
