...
conn.level = global.db.data.users[m.sender]
  ...
  if (global.db.data.users[m.sender].phonix == 0) return m.reply(`Kamu belum memiliki Phonix Silahkan beli dulu..\n\nKetik: *${usedPrefix}petshop phonix* untuk membeli phonix`)
  ...
  m.reply(`*Pet Kamu* (🦅phonix level ${global.db.data.users[m.sender].phonix}) ⚔️menantang 🦅phonixnya *${conn.getName(lawan)}* (🦅phonix level ${global.db.data.users[lawan].phonix}) lagi berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang🎮.`)
